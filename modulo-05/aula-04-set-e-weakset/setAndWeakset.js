...
// nao tem get, então você pode saber se o item está ou nao no array e é isso.
// na documentação tem exemplos sobre como fazer uma interseção, saber o que tem em uma lista e nao
...
assert.deepStrictEqual(Array.from(intersection), ['Gohan'])

const difference = new Set([...users01].filter(user => !users02.has(user)))
assert.deepStrictEqual(Array.from(difference), ['Goku', 'Vegeta'])
